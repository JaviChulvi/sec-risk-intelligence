...
por código SIC, y escribe un fichero de texto (un CIK por línea) listo para
pasárselo a edgar-crawler en el parámetro `cik_tickers` de config.json.

Por qué así: edgar-crawler descarga por CIK/ticker, no por sector. Los índices
trimestrales de EDGAR no llevan el SIC, así que el filtrado sectorial se hace
ANTES, construyendo aquí la lista de CIKs.

Requisitos: requests  ->  pip install requests
Política de acceso de la SEC: max 10 req/s y User-Agent descriptivo obligatorio.
"""
...
# Códigos SIC del universo bancario (instituciones de depósito + holdings).
BANK_SIC_CODES = [
...
PAGE_SIZE = 100               # máximo que admite browse-edgar por página
SLEEP_SECONDS = 0.15          # ~6-7 req/s, holgado bajo el límite de 10 req/s
...
def fetch_ciks_for_sic(sic: str) -> set[str]:
    """Devuelve el conjunto de CIKs (10 dígitos) que tienen filings 10-K
    bajo un código SIC, paginando hasta agotar resultados."""
    ciks: set[str] = set()
    start = 0
    while True:
        params = {
        ...
        new = page - ciks
        if not new:                # no hay CIKs nuevos -> fin de la paginación
            break
        ciks |= new
        start += PAGE_SIZE
...
def main() -> None:
    ...
    # edgar-crawler acepta un fichero con un CIK por línea en `cik_tickers`.
    with open(OUTPUT_FILE, "w") as f:
        for cik in sorted(all_ciks, key=int):
            f.write(str(int(cik)) + "\n")

    print(f"\nTotal de bancos únicos: {len(all_ciks)}")
    print(f"Escrito en: {OUTPUT_FILE}")
    print("Pásaselo a edgar-crawler -> config.json -> \"cik_tickers\": \"bank_ciks.txt\"")
